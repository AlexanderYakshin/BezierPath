...
namespace BezierPath
{
	...
	{
		[SerializeField]
		private BezierCalculatePathMode _calculatePathMode;
		[SerializeField]
		...
		private void SetLengths()
		{
			...
			var all = lengths.Sum();
			_curveLength = lengths.Select(length => length / all).ToArray();
		}

		private float[] GetLengths()
		{
			int count = _points.Length / 3;
			var lengths = new float[] { };
			Array.Resize(ref lengths, count);
			for (int j = 0; j < count; j++)
			{
				var i = j * 3;
				var length = BezierHelper.GetCurveLength(_points[i], _points[i + 1], _points[i + 2], _points[i + 3]);
				lengths[j] = length;
			}
			return lengths;
		}
		...
		public Vector3 GetPoint(float t)
		{
			...
			else
			{
				...
				if (_calculatePathMode == BezierCalculatePathMode.DependOnPath)
				{
				...
				else if (_calculatePathMode == BezierCalculatePathMode.Constantly)
				{
					var length = 0f;
					while (i < _curveLength.Length && length < t)
					{
						length += _curveLength[i];
						if (length < t)
							i++;
					}
					var previousLength = i > 0 ? length - _curveLength[i] : 0f;
					t = (t - previousLength) / _curveLength[i];
					i *= 3;
				}
			}
			return transform.TransformPoint(BezierHelper.GetPoint(_points[i], _points[i + 1], _points[i + 2], _points[i + 3], t));
		}
		...
		public Vector3 GetVelocity(float t)
		{
			...
			return transform.TransformPoint(BezierHelper.GetFirstDerivative(_points[i], _points[i + 1], _points[i + 2], _points[i + 3], t)) - transform.position;
		}
